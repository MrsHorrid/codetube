...
exports.generateAllLessons = generateAllLessons;
...
async function callLessonLLM(prompt) {
...
// ─── Prompt Template ──────────────────────────────────────────
function buildLessonPrompt(lesson, topic, voice) {
    const voiceStyle = {
...
function buildTypeEvents(code, startMs, language) {
    const events = [];
    let cursor = startMs;
...
function assembleRecordingEvents(codeExamples, narrationSegments, checkpoints, audioUrls) {
    const events = [];
...
function mockLessonResponse(prompt) {
...
// ─── Main Export ──────────────────────────────────────────────
async function generateLesson(lesson, topic, voice, audioUrls = new Map()) {
    ...
    const recordingEvents = assembleRecordingEvents(parsed.codeExamples, parsed.narrationSegments, checkpointsWithExercises, audioUrls);
    const durationMs = recordingEvents.length > 0
 ...
 */
async function generateAllLessons(curriculum, voice, onProgress) {
    const lessons = [];
...
//# sourceMappingURL=lessonGenerator.js.map