...
import { Curriculum, CurriculumLesson, SkillLevel, VoicePersona } from '../../types/ai';
...
function getLLMConfig(): LLMConfig {
  const provider = (process.env.AI_PROVIDER as LLMConfig['provider']) || 'mock';
  return {
    provider,
...
function buildCurriculumPrompt(topic: string, level: SkillLevel, voice: VoicePersona, maxLessons: number): string {
  const voiceDescriptions: Record<VoicePersona, string> = {
...
function applyProgressiveDifficulty(lessons: CurriculumLesson[], baseLevel: SkillLevel): CurriculumLesson[] {
...
export async function generateCurriculum(
  ...
  const progressiveLessons = applyProgressiveDifficulty(
    parsed.lessons.slice(0, maxLessons),
    level
  );
